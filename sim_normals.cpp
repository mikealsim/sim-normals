 ...
 *   return normal_map
*/
cv::Mat CreateNormal( cv::Mat gray ){    
    auto ddepth = CV_32F;
    std::vector<cv::Mat> mat_vec(3);
    /// Gradient X
    cv::Scharr( gray, mat_vec[2], ddepth, 1, 0,  .25, 0.0, cv::BORDER_DEFAULT );
    /// Gradient Y
    cv::Scharr( gray, mat_vec[1], ddepth, 0, 1,  .25, 0.0, cv::BORDER_DEFAULT );
    mat_vec[2] *= -1.0;
    mat_vec[1] *= -1.0;
...
bool MakeNormals(const cv::Mat& image, double min_detail, double max_detail, cv::Mat& normal){
    ...
    normal = CreateNormal(gray);
    int count =0;
    // pyramid processing
    while( gray.cols/2 > 3 && gray.rows/2 > 3){        
        resize(gray, gray, cv::Size( gray.cols/2, gray.rows/2), 0,0, cv::INTER_AREA );
        cv::Mat normal_pry = CreateNormal(gray);
        resize(normal_pry, normal_pry, cv::Size( image.cols, image.rows), 0,0, cv::INTER_CUBIC );
        // future: optional weighting based on pryimid level
        //addWeighted(normal, .85, normal_pry, 0.15 , 0.0, normal, CV_32F);
        normal += normal_pry;
        count++;
        if(max_detail > 0.0 && std::pow(2.0, count) > max_detail) break;
    }
    normal /= count;
